...
namespace API.Controllers
{
    public class AdminController : BaseApiController
    {
        ...
        {
            var photoToApprove = await _unitOfWork.PhotoRepository.GetPhotoById(id);

            photoToApprove.IsApproved = true;

            var user = await _unitOfWork.PhotoRepository.GetUserByPhotoId(id);
        ...
        {
            var photoToReject = await _unitOfWork.PhotoRepository.GetPhotoById(id);

            if (photoToReject.PublicId != null)
            {
                var result = await _photoService
                    .DeletePhotoAsync(photoToReject.PublicId);
                
                if (result.Result == "ok")
                {
                    _unitOfWork.PhotoRepository.RemovePhoto(photoToReject);
                }
            }
            else
            {
